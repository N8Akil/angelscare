...
export function TrustBadges({ className }: { className?: string }) {
    return (
        <div className={cn("flex flex-wrap items-center gap-4 md:gap-6", className)}>
            ...
            {/* BBB A+ Rating Badge */}
            <div className="flex items-center gap-3 bg-gold/10 border border-gold/40 rounded-full px-5 py-3 transition-colors cursor-help group" title="Better Business Bureau A+ Accredited">
                <div className="relative">
                    <Star className="w-6 h-6 text-gold fill-gold" />
                </div>
                <div className="flex flex-col">
                    <span className="text-[10px] uppercase tracking-widest text-navy/60 font-medium group-hover:text-navy transition-colors">BBB Accredited</span>
                    <span className="text-sm font-bold text-navy leading-none">A+ Rating</span>
                </div>
            </div>
